        ...
        if (!upgradeHeader || upgradeHeader !== 'websocket') {
            ...
            if (apiPath === 'api/uuid' || apiPath === 'api/config') {
                ...
                if (isDefaultUUID) {
                    ...
                    if (twoProxyConfig) {
                        configResponse.two_proxy = twoProxyConfig;  // returnFulltwoProxyconfig
                    }
                }
...
async function readconfig_JSON(env, hostname, userID, path, resetCfg = false) {
    ...
    const ECHLINKparam = config_JSON.ECH ? `&ech=${encodeURIComponent('cloudflare-ech.com+' + ECH_DOH)}` : '';
    config_JSON.LINK = `${config_JSON.protocolType}://${userID}@${host}:443?security=tls&type=${config_JSON.transport + ECHLINKparam}&host=${host}&fp=${config_JSON.Fingerprint}&sni=${host}&path=${encodeURIComponent(config_JSON.enable0RTT ? config_JSON.PATH + '?ed=2560' : config_JSON.PATH) + tlsFragmentparam}&encryption=none${config_JSON.skipCertVerify ? '&insecure=1&allowInsecure=1' : ''}#${encodeURIComponent(config_JSON.subGenerator.SUBNAME)}`;
